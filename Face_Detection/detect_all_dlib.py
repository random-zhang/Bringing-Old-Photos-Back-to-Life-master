...
def _standard_face_pts():  # 将一维数组转为5维
    pts = (
...
def search(face_landmarks):  # 人脸特征
...
def compute_transformation_matrix(img, landmark, normalize, target_face_scale=1.0):
    ...
    h, w, c = img.shape  # c 位深  计算机中用于表示颜色的位bit
    if normalize == True:
        """
        将特征的h列
        """
        landmark[:, 0] = landmark[:, 0] / h * 2 - 1.0  # x[:, n]表示取所有 x子元素（子集合）的第n个子子元素 (第n列）
        landmark[:, 1] = landmark[:, 1] / w * 2 - 1.0  # x[n,:]表示在n个数组（维）中取全部数据，直观来说，x[n,:]就是取第n集合的所有数据,
        # x[:,m:n]，即取所有数据集的第m到n-1列数据
    # print(landmark)
    affine = SimilarityTransform()  # 仿射   相似变换：等距变换+均匀尺度缩放，所谓等距变换就是平移+旋转变换。
