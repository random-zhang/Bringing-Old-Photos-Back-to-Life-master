...
def calculate_cdf(histogram):
    """
    This method calculates the cumulative（累积） distribution function
    :param array histogram: The values of the histogram（柱状图）
    :return: normalized_cdf: The normalized cumulative distribution function
    :rtype: array
    """
    # Get the cumulative sum of the elements
    cdf = histogram.cumsum() #（累计和）cdf 累积分布函数

    # Normalize the cdf （标准化）
    normalized_cdf = cdf / float(cdf.max()) #把累计直方图的比例化到近似直方图
...
def match_histograms(src_image, ref_image):
    ...
    :rtype: image (array)
    """
    # Split the images into the different color channels（将图像分割成不同的颜色通道）
    # b means blue, g means green and r means red
    src_b, src_g, src_r = cv2.split(src_image)
    ref_b, ref_g, ref_r = cv2.split(ref_image)

    # Compute the b, g, and r histograms separately #分别计算b、g、r直方图
    # The flatten() Numpy method returns a copy of the array c  flatten（）函数 返回c数组的副本   压缩成1维
    # collapsed into one dimension.
    src_hist_blue, bin_0 = np.histogram(src_b.flatten(), 256, [0, 256])#生成直方图   a [19 30 15 16 20]   b[ 0. 0.2  0.4  0.6  0.8  1. ] a为对应b区间的个数
    ...
    src_cdf_blue = calculate_cdf(src_hist_blue) #src_hist_blue为柱状图每个柱子的值
...
#已经算出累计分布函数
    # Make a separate lookup table for each color 为每种颜色确立检查表
